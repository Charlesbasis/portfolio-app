...
if [ -n "$NEXTJS_URL" ]; then
  ...
  # Plugin v1.1.0 expects: nextjs_url, webhook_secret, cooldown, max_retries, debug_mode
  wp option update next_revalidate_settings "{\"nextjs_url\":\"${NEXTJS_URL}\",\"webhook_secret\":\"${WORDPRESS_WEBHOOK_SECRET:-}\",\"cooldown\":2,\"max_retries\":3,\"debug_mode\":true}" --format=json --allow-root
