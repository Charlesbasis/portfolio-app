...
/**
 * Fallback user type configurations
 * These match the seeded database but should NOT be the primary source
 */
export const FALLBACK_USER_TYPES: Record<string, UserTypeConfig> = {
  student: {
    value: 'student',
    label: 'Student',
...
/**
 * Helper functions for fallback config
 * @deprecated Use useUserTypeConfig hook instead for API-driven configs
 */
export const getUserTypeConfig = (userType: string): UserTypeConfig | null => {
  return FALLBACK_USER_TYPES[userType] || null;
};

export const getUserTypeOptions = () => {
  return Object.values(FALLBACK_USER_TYPES).map(type => ({
    value: type.value,
    label: type.label,
...
export { buildUserTypeConfig, userTypeHelpers } from './userTypeBuilder';