...
export async function POST(request: NextRequest) {
  try {
    ...
    // Support both old (v1.0.5) and new (v1.1.0) plugin payload formats
    const contentType = requestBody.type || requestBody.contentType;
    const data = requestBody.data || {};
    const contentId = data.id || requestBody.contentId;

    if (!contentType) {
      return NextResponse.json(
        { message: "Missing content type", received: requestBody },
        { status: 400 }
      );
    }
